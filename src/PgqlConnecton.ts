...
import { JavaPgqlPreparedStatement, PgqlPreparedStatement } from "./PgqlPreparedStatement";
...
export class PgqlConnection {
  ...
  private constructor(connection: OracleConnection) {
    this.pgqlConn = javaPgqlConnection.getConnectionSync(connection.getRawConnection());
  }
  ...
  async prepareStatement(pgql: string): Promise<PgqlPreparedStatement> {
    const pstmt: JavaPgqlPreparedStatement = await this.pgqlConn.prepareStatement(pgql);
